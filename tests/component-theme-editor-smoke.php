...
foreach ( array( 'Flat', 'Elevated', 'Glass', 'Minimal', 'Filled', 'Outline', 'Soft', 'Transparent', 'Standard', 'Alternate', 'Highlight' ) as $style ) {
...
adam_ui_editor_assert( false !== strpos( $script, 'applyAutomaticContrast' ), 'Automatic component contrast is missing from live preview.' );
adam_ui_editor_assert( false !== strpos( $repository, 'apply_styles' ), 'Saved component styles are not resolved into design tokens.' );
adam_ui_editor_assert( false !== strpos( $components . $preview, '--adam-card-backdrop' ), 'Card visual presets are not bound to shared CSS.' );
